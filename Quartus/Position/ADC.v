	...
	module ADC(input wire clk, input wire start, input sdo, output wire ready, output reg[95:0] data, output reg convst, output reg sck, output reg sdi);
      ...
      always @(posedge clk) begin
        ...
        sck = (s80 >= 64 && s80 < 76 && (s160 & 1));
		  ...
		  // Write the control bits to sdi at the appropriate times when sck is low
		  // and hold to allow it to be read by the ADC when sck rises
        case (state)
        ...
        endcase
		  ...
		  // Save sdo to the appropriate index of data on the posedge of sck
		  // to ensure the value in the ADC has settled
        case (state)
		  // FOR TESTING ONLY, SET ALL LEDS to 1
          129: data[0] = sdo;
          131: data[1] = sdo;
          133: data[2] = sdo;
          135: data[3] = sdo;
